...
#        '''Return a version of this FST with deadends eliminated.'''
#        fst = self.copy(label=label)
#        deleted = 0
#        
...
#                deleted += 1
#                if trace and deleted % 5000 == 0:
#                    print('  Deleted', deleted, 'states')
#                fst.del_state(state)
#
#        if trace and deleted > 0:
#            print('Deleted', deleted, 'total states from', self.label)
#        return fst
...
#       if trace:
#            print('  Trimming; {} initial states'.format(len(to_delete)))

        ## Seems ALL states have a path from the initial state...
#        # Determine whether there is a path from the initial node to
#        # each node.
...
#        while queue:
#            state = queue.pop()
#            dsts = [self.dst(arc) for arc in self.outgoing(state)]
#            queue += [s for s in dsts if s not in path_from_init]
#            path_from_init.update(dsts)
...
#        # to a final state.
#        n_states = 0
#        # We need a copy of self.states() because some will be deleted
#        for state in states_copy:
#            n_states += 1
#            if trace and n_states % 5000 == 0:
#                print('  Checked {} deletable states'.format(n_states))
#            if state not in path_to_final:
##            if not (state in path_from_init and state in path_to_final):
#                deleted += 1
#                if trace and deleted % 5000 == 0:
#                    print('  Deleted', deleted, 'states')
...
#            return FST.restore_parse(directory, filename, weighting=weighting,
#                                     seg_units=seg_units,
#                                     create_weights=create_weights)
        if not empty:
...
#            if os.path.exists(os.path.join(directory, filename)):
#                if verbose:
#                    print('  Restoring FST', empty_name, 'from FST file')
#                return FST.restore_parse(directory, filename, weighting=weighting,
#                                         seg_units=seg_units,
#                                         create_weights=create_weights)
